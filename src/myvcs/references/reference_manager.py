...
class ReferenceManager:
    ...
    def current_branch(self) -> str:
        ...
        try:
            ...
            if not head_content.startswith(
                head_prefix
            ):
                raise ReferenceError(
                    "HEAD is not a symbolic reference."
                )
            ...
            if not reference.startswith(
                ...
                raise ReferenceError(
                    f"Invalid HEAD reference: {reference}"
                )
    ...
    def current_branch_file(self):
        ...
        return (
            self.repository.heads_directory
            / branch_name
        )
    ...
    def get_branch_commit(
        ...
        try:
            branch_file = self.get_branch_file(
                branch_name
            )
            ...
            raise ReferenceError(
                f"Unable to read branch reference: "
                f"{branch_name}"
            ) from exc
    ...
    def update_head(
        ...
        try:
            ...
            LOGGER.exception(
                "Unable to update branch reference."
            )
    ...
    def list_branches(self) -> list[str]:
        ...
        try:
            ...
            return sorted(
                path.name
                for path in heads_directory.iterdir()
                if path.is_file()
            )
    ...
    def create_tag(
        ...
        tag_file = (
            self.repository.tags_directory
            / tag_name
        )
    ...
    def list_tags(self) -> list[str]:
        ...
        try:
            ...
            return sorted(
                path.name
                for path in tags_directory.iterdir()
                if path.is_file()
            )
