...
class RepositoryManager:
    ...
    def __init__(
        ...
        self.metadata_directory = self.working_directory / metadata_directory_name

        self.objects_directory = self.metadata_directory / objects_directory_name

        self.references_directory = self.metadata_directory / references_directory_name

        self.heads_directory = self.references_directory / heads_directory_name

        self.tags_directory = self.references_directory / tags_directory_name

        self.index_file = self.metadata_directory / index_file_name

        self.head_file = self.metadata_directory / head_file_name

        self.repository_config_file = (
            self.metadata_directory / repository_config_file_name
        )
    ...
    def require_repository(self) -> None:
        ...
        if not self.exists():
            raise RepositoryError(
                f"No MyVCS repository found in {self.working_directory}"
            )
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            self.head_file.write_text(
                f"{head_prefix}{heads_prefix}{default_branch}\n",
                encoding=UTF8_ENCODING,
            )

            default_branch_file = self.heads_directory / default_branch
            ...
            raise RepositoryError("Unable to initialize repository.") from exc
    ...
    def relative_path(
        ...
        try:
            return str(file_path.resolve().relative_to(self.working_directory))

        except ValueError as exc:
            raise RepositoryError(f"Path is outside repository: {file_path}") from exc
