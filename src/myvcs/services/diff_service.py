...
class DiffService:
    ...
    def diff(self) -> str:
        ...
        try:
            ...
            if not head_commit_id:
                LOGGER.info("No commits exist; no diff available.")
                return ""

            # ObjectStore.read() returns raw serialized bytes.
            commit_data = self.object_store.read(head_commit_id)

            tree_id = self._get_tree_id(commit_data)

            head_files = self._flatten_tree(tree_id)

            working_files = self._working_files()

            output: list[str] = []

            all_paths = sorted(set(head_files) | working_files)

            for relative_path in all_paths:
                head_object_id = head_files.get(relative_path)

                file_path = self.repository.working_directory / Path(relative_path)

                # File existed in HEAD but has been deleted.
                if head_object_id is not None and relative_path not in working_files:
                    output.append(f"deleted: {relative_path}")
                    continue

                # File does not exist in HEAD.
                if head_object_id is None:
                    output.append(f"added: {relative_path}")
                    continue

                # File exists in both HEAD and working tree.
                try:
                    old_blob_data = self.object_store.read(head_object_id)
                except Exception:
                    LOGGER.exception(
                ...
                old_blob_text = self._extract_object_payload(old_blob_data).decode(
                    "utf-8",
                    errors="replace",
                )
            ...
            LOGGER.exception("Unable to generate working-tree diff.")
            raise
    ...
    def _extract_object_payload(
        ...
        if not isinstance(object_data, bytes):
            raise TypeError("Invalid VCS object data.")
    ...
    def _working_files(self) -> set[str]:
        ...
        for path in self.repository.working_directory.rglob("*"):
            ...
            relative = self.repository.relative_path(path)
    ...
    def _flatten_tree(
        ...
        tree_payload = self._extract_object_payload(tree_data)
        ...
        for entry in tree.entries:
            ...
            if entry.object_type == tree_type:
                ...
                result[relative_path] = entry.object_id
